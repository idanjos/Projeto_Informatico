...
char    **argv;
{
    ...
    printf("%d\n",sfecg); //Returning freq so theres no dependecy elsewhere
    int units = 0;
    for(units = 0; units<=N_heartbeats;units++){
     
      read_file();
      if(stimulation==1){
        //hrmean = hrmean + (((float)units/(float)N_heartbeats)* (desired_heartbeat - hrmean));
        hrmean = desired_heartbeat;
       // printf("%f %f\n",hrmean,(((float)units/(float)N_heartbeats)* (desired_heartbeat - hrmean)));
      }
        
      
      dorun();
      
     // usleep((int)(1000*3000));
    }
    ...
    while ((read = getline(&line, &len, fpp)) != -1) {
       ...
       // printf("%s", line);
        ...
        while (pch != NULL)
        {
          ...
          if(i<6){
          ti[i] = strtod(pch,NULL);
        }else if(i>=6 && i < 11){
          ai[i-5] = strtod(pch,NULL);

        }else if(i>=11 && i<16){
          bi[i-10] = strtod(pch,NULL);
        }else{
          switch(i){
            case 16:{
              if(flag_read == 0)
                N_heartbeats = strtod(pch,NULL);
              else{
                int temp;
                temp = strtod(pch,NULL);
              }
             // printf("%d",N_heartbeats);
              flag_read = 1;
              break;
            }
            case 17:{
              sfecg = strtod(pch,NULL);
              break;
            }
            case 18:{
              sf = strtod(pch,NULL);
            ...
            case 19:{
              Anoise = strtod(pch,NULL);
              break;
            }
            case 20:{
              hrmean = strtod(pch,NULL);
              break;
            }
            case 21:{
              hrstd = strtod(pch,NULL);
              break;
            }
            case 22:{
              flo = strtod(pch,NULL);
            ...
            case 24:{
              flostd = strtod(pch,NULL);
              break;
            }
            case 25:{
              fhistd = strtod(pch,NULL);
              break;
            }
            case 26:{
              lfhfratio = strtod(pch,NULL);
              break;
            }
            case 27:{
              seed = strtod(pch,NULL);
              break;
            }
            case 28:{
              total_time = strtod(pch,NULL);
              if(total_time > 0){
                //N = total_time*(hrmean/60);
              }
              break;
            }
            case 29:{
            ...
            case 30:{
              desired_heartbeat = strtod(pch,NULL);
              break;
            }
            case 31:{
              stimulation = strtod(pch,NULL);
              break;
            }
...
int dorun()
{
   ...
   /* P            Q            R           S           T        */
    ...
    //ai[1]=1.2;   ai[2]=-5.0;  ai[3]=30.0; ai[4]=-7.5; ai[5]=0.5;
    //bi[1]=0.25;  bi[2]=0.1;   bi[3]=0.1;  bi[4]=0.1;  bi[5]=0.3;
  ...
  //hrmean *= 0.5;
   ...
   int flag = 1;
   float diff = (double)rand() / (double)((unsigned)RAND_MAX + 1)*Anoise;
    for(i=1;i<=Nts;i++){
    // zts[i] += Anoise*(2.0*ran1(&rseed) - 1.0);
      float signal =  (double)rand() / (double)((unsigned)RAND_MAX + 1);
      
      if (signal < 0.5 && (i % seed) == 0){
        flag = -1;
        diff = (double)rand() / (double)((unsigned)RAND_MAX + 1)*Anoise;
      }
   ...
   for(i=1;i<=Nts;i++){
    printf(",%f",zts[i]);
    if(total_time>0)
      usleep((int)(floor(1000/sfecg)*total_time));
    //fflush(stdout);
   }
